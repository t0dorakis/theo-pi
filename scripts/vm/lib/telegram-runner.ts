...
export function createTelegramRunner(options: {
  ...
  async function runOnce() {
    ...
    if (!job) return false
    // Already delivered — release by marking failed to clear the lease, then skip.
    if (job.telegramDeliveredAt) {
      await options.queue.failJob(job.id, "already delivered").catch(() => {})
      return false
    }
