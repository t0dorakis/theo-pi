...
export function createTelegramPoller(options: {
  queue: { enqueueJob(input: { chatId: string; prompt: string }): Promise<{ id: string }> }
  ...
  async function handleMessage(message: TelegramMessage) {
    ...
    if (textValue === "/reload") {
      await options.telegram.sendMessage(chatId, await options.commands.reload())
      return
    }

    if (textValue === "/restart-gateway") {
      await options.telegram.sendMessage(chatId, await options.commands.restartGateway())
      return
    }
    ...
    await options.onJobDelegation?.(job.id)
  }
