...
if ! command -v acpx >/dev/null 2>&1; then
  ...
  echo "==> acpx already installed: $(acpx --version || true)"
fi
...
chmod +x "$SCRIPT_DIR"/pi-worker-supervisor "$SCRIPT_DIR"/pi-worker-start "$SCRIPT_DIR"/pi-worker-status "$SCRIPT_DIR"/pi-worker-restart "$SCRIPT_DIR"/pi-worker-stop "$SCRIPT_DIR"/pi-worker-checkpoint "$SCRIPT_DIR"/pi-worker-tail-logs "$SCRIPT_DIR"/pi-worker-verify-runtime "$SCRIPT_DIR"/pi-worker-fail-inject "$SCRIPT_DIR"/pi-worker-runtime-checklist "$SCRIPT_DIR"/pi-worker-submit-job "$SCRIPT_DIR"/pi-worker-run-job "$SCRIPT_DIR"/pi-worker-gateway "$SCRIPT_DIR"/pi-worker-telegram-bot "$SCRIPT_DIR"/pi-worker-telegram-runner "$SCRIPT_DIR"/pi-worker-acp "$SCRIPT_DIR"/pi-worker-verify.sh "$SCRIPT_DIR"/pi-worker-supervisor-smoke-test "$SCRIPT_DIR"/pi-worker-gateway-smoke-test "$SCRIPT_DIR"/pi-worker-acpx-smoke-test "$SCRIPT_DIR"/pi-worker-gateway.ts "$SCRIPT_DIR"/pi-worker-telegram-bot.ts "$SCRIPT_DIR"/pi-worker-telegram-runner.ts "$SCRIPT_DIR"/pi-worker-acp-stdio.ts "$SCRIPT_DIR"/pi-worker-acp-stdio-smoke-test.ts "$SCRIPT_DIR"/pi-worker-submit-job.ts "$SCRIPT_DIR"/pi-worker-run-job.ts
